...
class OrderService {
  ...
  async createOrder(order, productId) {
    ...
    try {
      ...
      const [order] = await connection.query(query, [subtotal, total, purchase_date, state_id, user_id, quantity]);

      const orderId = order.insertId;
      const query2 = 'INSERT INTO customer_order_product (customer_order_id, product_id) VALUES (?, ?)';
      await connection.query(query2, [orderId, productId]);
  ...
  async findAllOrdersByUser(idUser) {
    try {
      const query =
        'SELECT co.id AS order_id, p.id AS product_id, co.*, p.* FROM customer_order co INNER JOIN customer_order_product cop ON co.id = cop.customer_order_id INNER JOIN product p ON cop.product_id = p.id WHERE co.user_id = ?';
